...
import { Mic, Send, CheckCircle, Square, Briefcase, Coffee, Stethoscope, Loader2, Sparkles, Plane, Hotel, Users, MapPin, BookOpen, RefreshCw, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f0] font-sans text-gray-800 flex flex-col md:flex-row overflow-hidden">
      {/* Sidebar */}
      <div className="w-full md:w-80 bg-white border-r border-gray-200 p-6 flex flex-col overflow-y-auto h-screen custom-scrollbar">
        <div className="flex items-center space-x-3 mb-8 shrink-0">
          <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-md">
            <Sparkles className="w-6 h-6" />
          </div>
          <h1 className="text-xl font-bold text-gray-900">LingoTutor</h1>
        </div>

        <div className="mb-8 shrink-0">
          <button 
            onClick={() => setIsStudySheetOpen(true)}
            className="w-full bg-indigo-50 hover:bg-indigo-100 border border-indigo-100 text-indigo-700 rounded-xl p-4 flex items-center justify-between transition-colors shadow-sm"
          >
            <div className="flex items-center">
              <BookOpen className="w-5 h-5 mr-3" />
              <div className="text-left">
                <h3 className="font-bold text-sm">Study Materials</h3>
                <p className="text-xs text-indigo-500/80">Idioms, Sentences & Grammar</p>
              </div>
            </div>
        ...
        <div className="shrink-0 pb-6">
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Live Roleplay</h3>
          <div className="space-y-3">
            {Object.entries(groupedRoleplays).map(([category, roleplays]) => (
              <div key={category} className="mb-2">
                <button
                  onClick={() => toggleCategory(category)}
                  className="w-full flex items-center justify-between p-2 text-sm font-semibold text-gray-600 hover:bg-gray-50 rounded-lg transition-colors"
                >
                  <span>{category}</span>
                ...
                {expandedCategories[category] && (
                  <div className="mt-1 space-y-2 pl-1 animate-in slide-in-from-top-2 duration-200">
                    {roleplays.map((rp, idx) => (
                      <button key={`${category}-${idx}`} onClick={() => setLiveScenario(rp.title)} className="w-full flex items-center space-x-3 p-2.5 rounded-xl hover:bg-gray-50 border border-gray-100 transition-colors text-left shadow-sm bg-white">
                        <div className="bg-indigo-50 text-indigo-600 p-1.5 rounded-lg w-8 h-8 flex items-center justify-center text-base shrink-0">{rp.emoji}</div>
                        <span className="font-medium text-gray-700 text-sm">{rp.title}</span>
                      </button>
                    ))}
                  </div>
                )}
            ...
            <button 
              ...
              className="w-full flex items-center justify-center space-x-2 p-3 rounded-xl border border-dashed border-indigo-300 text-indigo-600 hover:bg-indigo-50 transition-colors disabled:opacity-50 mt-4"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-2xl p-4 ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-sm shadow-md' : 'bg-white border border-gray-200 text-gray-800 rounded-tl-sm shadow-sm'}`}>
        ...
        <div className="p-4 bg-white border-t border-gray-200">
          <div className="max-w-4xl mx-auto flex items-end space-x-2">
            <button
              ...
              className={`p-3 rounded-xl flex-shrink-0 transition-colors shadow-sm ${isRecording ? 'bg-red-100 text-red-600 animate-pulse' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
              title="Click to speak"
            >
              {isRecording ? <Square className="w-6 h-6" /> : <Mic className="w-6 h-6" />}
            </button>
            <div className="flex-1 bg-gray-100 rounded-xl flex items-center px-4 py-2 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:bg-white transition-all shadow-sm">
              <input
                type="text"
                value={input}
                ...
                className="w-full bg-transparent border-none focus:outline-none py-2 text-gray-800 placeholder-gray-400"
